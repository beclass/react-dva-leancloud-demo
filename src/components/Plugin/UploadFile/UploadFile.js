    ...
    const uploadProps = {
      ...
      customRequest:(fileList)=>{
        ...
        tempAvFile.save().then(function(success) { 
             ...
             let url=success.attributes.url;
             let editList=this.state.fileList;
              if(Array.isArray(editList)) {
                  let urls=[url];
                  for(let i in editList){
                    urls.push(editList[i].url);
                  }
                  let fileList=this.initFiles(urls);

                  editList.push(fileItem);

                  onUpload(editList);

              }else{
                  const filesArr = url.split('/')
                  let file=[{ uid: -1, url: url, name: filesArr[filesArr.length - 1], status: 'done' }];
                  onUpload(file);
